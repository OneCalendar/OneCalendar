...
class ApplicationTest extends FunSuite with ShouldMatchers with Mockito with CollectionsUtils with PreviewJsonWriter {
    ...
    test("should find Nothing") {
        ...
        val dao = mock[SearchPreviewDao]
        when(dao.findPreviewByTag(Set("no", "match"))).thenReturn(Future(SearchPreview(0, Seq())))
    ...
    test("should find something with less than 3 elements") {
        ...
        val dao = mock[SearchPreviewDao]
        val size: Int = 2
        when(dao.findPreviewByTag(Set("with","match")))
            .thenReturn(Future(SearchPreview(size, Seq(
